...
function onMouseDown (event) {
  ...
  if (!isDragging && (e.button === undefined || e.button === 0)) {
    ...
    if (grabbedElement) {
      ...
      const hasOptions = Object.keys(container.getOptions()).length;
      if (container && hasOptions) {
        const {
          dragClass,
          containerClass,
          dragHandleSelector,
        ...
        if (dragClass) {
          Utils.addClass(e.target, dragClass);
        }
        if (containerClass) {
          const parent = Utils.getParent(
            grabbedElement,
            '.' + constants.containerClass
          );
          Utils.addClass(parent, containerClass);
        }
        let startDrag = true;
        if (
          dragHandleSelector &&
          !Utils.getParent(e.target, dragHandleSelector)
        ) {
          startDrag = false;
        }
        if (
          nonDragAreaSelector &&
          Utils.getParent(e.target, nonDragAreaSelector)
        ) {
          startDrag = false;
        }
        if (startDrag) {
          ...
          const onMouseUp = () => {
            ...
            if (container.getOptions().dragClass) {
              Utils.removeClass(e.target, container.getOptions().dragClass);
            }
...
function initiateDrag (position, cursor) {
  if (grabbedElement !== null) {
    ...
    sourceContainerLockAxis = container.getOptions().lockAxis
      ? container.getOptions().lockAxis.toLowerCase()
      : null;
